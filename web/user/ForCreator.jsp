                                ...
                                <center>
                                    <table>
                                        ...
                                        <tbody>
                                            ...
                                            <c:forEach var="c" items="${clubCreator}" begin="${startIndex}" end="${endIndex}" varStatus="loop">
                                                <tr>
                                                    <td>${loop.index + 1}</td>
                                                    <td>${c.code}</td>
                                                    <td>${c.name}</td>
                                    ...
                                    <c:choose>
                                        <c:when test="${currentPage > 1}">
                                            <a href="<%=request.getContextPath()%>/user?command=ForCreator&page=1"><<</a>
                                            <a href="<%=request.getContextPath()%>/user?command=ForCreator&page=${previousPage}"><</a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="<%=request.getContextPath()%>/user?command=ForCreator&page=1"><<</a>
                                            <a href="<%=request.getContextPath()%>/user?command=ForCreator&page=${currentPage}"><</a>
                                        </c:otherwise>
                                    </c:choose>
                                    ...
                                    <c:choose>
                                        <c:when test="${currentPage < totalPages}">
                                            <a href="<%=request.getContextPath()%>/user?command=ForCreator&page=${nextPage}">></a>
                                            <a href="<%=request.getContextPath()%>/user?command=ForCreator&page=${totalPagess + 1}">>></a>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="<%=request.getContextPath()%>/user?command=ForCreator&page=${currentPage}">></a>
                                            <a href="<%=request.getContextPath()%>/user?command=ForCreator&page=${totalPagess + 1}">>></a>
