...
public class UserServlet extends HttpServlet {
    ...
    private void GetCookie(HttpServletRequest request, HttpServletResponse response)
        ...
        User user = userDAO.checkUserLogin(cAcc, cPass);
        if (user != null) {
            if (cAcc.equals(user.getEmail()) && cPass.equals(user.getPass())) {
    ...
    private void Login(HttpServletRequest request, HttpServletResponse response)
        ...
        User block = userDAO.checkUserBlock(acc, pass);
        Admin admin = new Admin(acc, pass);
        boolean check = AdminDao.login(admin);

        if (block != null) {
            request.getSession().setAttribute("warning", "Your account has been locked!");
            ...
            if (user == null && check == false) {
                ...
                if (remember != null && remember.equals("on")) {
                    Cookie rememberAcc = new Cookie("rAcc", acc);
                    Cookie rememberPass = new Cookie("rPass", pass);
                    rememberAcc.setMaxAge(24 * 60 * 60 * 30);
                    rememberPass.setMaxAge(24 * 60 * 60 * 30);
                    response.addCookie(rememberAcc);
    ...
    private void Logout(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
        HttpSession session = request.getSession();
        Cookie rememberAcc = new Cookie("rAcc", "");
        Cookie rememberPass = new Cookie("rPass", "");
        rememberAcc.setMaxAge(0);
    ...
    private void createLastSentEmailCookie(HttpServletResponse response) {
        long currentTimestamp = System.currentTimeMillis();
        Cookie cookie = new Cookie("last_sent_email", String.valueOf(currentTimestamp));
        cookie.setMaxAge(24 * 60 * 60 * 5);
        response.addCookie(cookie);
    }

    private void updateLastSentEmailCookie(HttpServletResponse response, long currentTimestamp) {
        Cookie cookie = new Cookie("last_sent_email", String.valueOf(currentTimestamp));
        cookie.setMaxAge(24 * 60 * 60 * 5);
        response.addCookie(cookie);
    }
    ...
    private void Register(HttpServletRequest request, HttpServletResponse response)
        ...
        if (acc == null || acc.isBlank() || pass == null || pass.isBlank() || repass == null || repass.isBlank()) {
            ...
            if (!pass.equals(repass)) {
                ...
                if (u == null) {
                    Cookie[] cookies = request.getCookies();
                    Cookie lastSentEmailCookie = findCookie(cookies, "last_sent_email");
                    if (lastSentEmailCookie != null) {
                        long lastSentTimestamp = Long.parseLong(lastSentEmailCookie.getValue());
                        long currentTimestamp = System.currentTimeMillis();
                        if (currentTimestamp - lastSentTimestamp < 15 * 60 * 1000) {
                            request.getSession().setAttribute("warning", "Please wait 15 minutes to re-register!");
                            response.sendRedirect("user?command=Regist");
                        } else {
                            ...
                            updateLastSentEmailCookie(response, currentTimestamp);
                        ...
                        session.setAttribute("recipientEmail", recipientEmail);
                        session.setAttribute("OTP", OTP);
                        createLastSentEmailCookie(response);
                        response.sendRedirect("user?command=rRegister");
                    }
                } else {
        ...
        if (recipientEmail == null || recipientEmail.isBlank()) {
            ...
            if (email == null) {
                ...
                if (lastSentEmailCookie != null) {
                    ...
                    long currentTimestamp = System.currentTimeMillis();
                    if (currentTimestamp - lastSentTimestamp < 15 * 60 * 1000) {
                        request.getSession().setAttribute("warning", "Please wait 15 minutes to resend!");
                        response.sendRedirect("user?command=rForgotPass");
                    } else {
                        SendEmail emailSender = new SendEmail();
    ...
    private void LoadEditProfile(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user == null) {
            ...
            User users = userDAO.getUserByID(ID);
            request.setAttribute("user", users);
            session.setAttribute("editProfile", "editProfile");
    ...
    private void ViewDetailsClubs(HttpServletRequest request, HttpServletResponse response)
        ...
        if (checkClubID == null || checkClubID.isBlank()) {
            ...
            if (!checkClubID.equals(clubID)) {
                response.sendRedirect("user?command=ViewDetailsClubs&cID=" + checkClubID);
            } else {
                if (user == null) {
                    response.sendRedirect("user?command=rLogin");
                } else if (clubID == null || clubID.isBlank()) {
    ...
    private void ViewDetailsJoins(HttpServletRequest request, HttpServletResponse response)
        ...
        if (checkMemberID == null || checkMemberID.isBlank()) {
            ...
            if (!checkMemberID.equals(memberID)) {
                response.sendRedirect("user?command=ViewDetailsJoins&mID=" + checkMemberID);
            } else {
                if (user == null) {
                    response.sendRedirect("user?command=rLogin");
                } else if (memberID == null || memberID.isBlank()) {
    ...
    private void ViewDetailsPosts(HttpServletRequest request, HttpServletResponse response)
        ...
        if (checkPostID == null || checkPostID.isBlank()) {
            ...
            if (!checkPostID.equals(postID)) {
                response.sendRedirect("user?command=ViewDetailsPosts&pID=" + checkPostID);
            } else {
                if (user == null) {
                    response.sendRedirect("user?command=rLogin");
                } else if (postID == null || postID.isBlank()) {
    ...
    private void ViewDetailsEvents(HttpServletRequest request, HttpServletResponse response)
        ...
        if (checkEventID == null || checkEventID.isBlank()) {
            ...
            if (!checkEventID.equals(eventID)) {
                response.sendRedirect("user?command=ViewDetailsEvents&eID=" + checkEventID);
            } else {
                if (user == null) {
                    response.sendRedirect("user?command=rLogin");
                } else if (eventID == null || eventID.isBlank()) {
    ...
    private void ClubsList(HttpServletRequest request, HttpServletResponse response)
        ...
        session.removeAttribute("cClubID");
        session.removeAttribute("cClubCreatorID");
        session.removeAttribute("createClub");
    ...
    private void ViewDetailsClub(HttpServletRequest request, HttpServletResponse response)
        ...
        session.removeAttribute("IsMember");
        User user = (User) session.getAttribute("account");
        String checkClubID = (String) session.getAttribute("cClubID");
        String checkClubCreatorID = (String) session.getAttribute("cClubCreatorID");

        UserDAO userDAO = new UserDAO();

        if (checkClubID == null || checkClubID.isBlank() || checkClubCreatorID == null || checkClubCreatorID.isBlank()) {
            if (user == null) {
                response.sendRedirect("user?command=rLogin");
                ...
                session.setAttribute("cClubID", clubID);
                session.setAttribute("cClubCreatorID", clubCreatorID);
                request.getRequestDispatcher("user/ViewDetailsClub.jsp").forward(request, response);
            }
        } else {
            if (!checkClubID.equals(clubID) || !checkClubCreatorID.equals(clubCreatorID)) {
                response.sendRedirect("user?command=ViewDetailsClub&cID=" + checkClubID + "&cCID=" + checkClubCreatorID);
            } else {
                if (user == null) {
                    response.sendRedirect("user?command=rLogin");
                } else if (clubID == null || clubID.isBlank() || clubCreatorID == null || clubCreatorID.isBlank()) {
    ...
    private void rCreateClub(HttpServletRequest request, HttpServletResponse response)
        ...
        session.removeAttribute("IsManager");
        session.removeAttribute("IsMember");
        session.setAttribute("createClub", "createClub");
...
//                int clubID = userDAO.checkClubID();
    ...
    private void JoinClub(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user == null) {
            ...
            User checkKickStatusExist = userDAO.checkKickStatusExist(clubID, user.getID());
...
//            request.setAttribute("clubID", clubID);
            ...
            if (!checkClubID.equals(clubID) || !checkClubCreatorID.equals(clubCreatorID)) {
                response.sendRedirect("user?command=IsCreator&cID=" + checkClubID + "&cCID=" + checkClubCreatorID);
            } else {
                if (user == null) {
                    response.sendRedirect("user?command=Home");
                } else if (clubID == null || clubID.isBlank() || clubCreatorID == null || clubCreatorID.isBlank()) {
...
//            request.setAttribute("clubID", clubID);
            ...
            if (!checkClubID.equals(clubID) || !checkClubCreatorID.equals(clubCreatorID)) {
                response.sendRedirect("user?command=IsManager&cID=" + checkClubID + "&cCID=" + checkClubCreatorID);
            } else {
                if (user == null) {
                    response.sendRedirect("user?command=Home");
                } else if (clubID == null || clubID.isBlank() || clubCreatorID == null || clubCreatorID.isBlank()) {
...
//            request.setAttribute("clubID", clubID);
            ...
            if (!checkClubID.equals(clubID) || !checkClubCreatorID.equals(clubCreatorID)) {
                response.sendRedirect("user?command=IsMember&cID=" + checkClubID + "&cCID=" + checkClubCreatorID);
            } else {
                if (user == null) {
                    response.sendRedirect("user?command=Home");
                } else if (clubID == null || clubID.isBlank() || clubCreatorID == null || clubCreatorID.isBlank()) {
    ...
    private void ClubPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (clubID != null) {
            ...
            ArrayList<Post> myPost = userDAO.getMyPost(memberID);
            if (post == null) {
                ...
                request.setAttribute("clubID", clubID);
                session.removeAttribute("createPost");
                request.getRequestDispatcher("user/ClubPost.jsp").forward(request, response);
            }
        } else {
            response.sendRedirect("user?command=Home");
...
//                request.setAttribute("joinEvents", joinEvents);
    ...
    private void ViewClubDetails(HttpServletRequest request, HttpServletResponse response)
        ...
        if (clubID != null) {
            ...
            request.getSession().removeAttribute("name");
            request.getSession().removeAttribute("description");
            session.removeAttribute("editClub");
    ...
    private void LoadEditClub(HttpServletRequest request, HttpServletResponse response)
        ...
        if (clubID != null) {
            ...
            request.setAttribute("club", clubs);
            request.setAttribute("clubID", clubID);
            session.setAttribute("editClub", "editClub");
...
//            userDAO.deleteClub(clubID);
    ...
    private void rCreatePost(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
        HttpSession session = request.getSession();
        session.setAttribute("createPost", "createPost");
...
//                    System.out.println(postID);
    ...
    private void PostManage(HttpServletRequest request, HttpServletResponse response)
        ...
        if (checkPostID == null || checkPostID.isBlank()) {
            ...
            if (!checkPostID.equals(postID)) {
                response.sendRedirect("user?command=PostManage&pID=" + checkPostID);
            } else {
                if (clubID != null) {
                    int memberID = userDAO.getMemberID(user.getID(), clubID);
                    int clubCreatorID = userDAO.getClubCreatorID(clubID);
        ...
        if (checkPostID == null || checkPostID.isBlank()) {
            if (clubID != null) {
                ...
                if (post == null) {
                    ...
                    request.setAttribute("clubID", clubID);
                    session.setAttribute("postID", postID);
                    session.setAttribute("editPost", "editPost");
                    request.getRequestDispatcher("user/EditPost.jsp").forward(request, response);
                }
            } else {
                response.sendRedirect("user?command=Home");
            ...
            if (!checkPostID.equals(postID)) {
                response.sendRedirect("user?command=LoadEditPost&pID=" + checkPostID);
            } else {
                if (clubID != null) {
                    Post post = userDAO.getPost(clubID, postID);
                    if (post == null) {
...
//            userDAO.insertNotification("Create Post", "Not Accepted", userID, java.sql.Date.valueOf(currentDate));
    ...
    private void ViewComment(HttpServletRequest request, HttpServletResponse response)
        ...
        HttpSession session = request.getSession();
        session.removeAttribute("postCommentID");
        User user = (User) session.getAttribute("account");
        String clubID = (String) session.getAttribute("clubID");
        String checkPostID = (String) session.getAttribute("postID");
...
//                request.setAttribute("notMyComments", notMyComments);
            ...
            if (!checkPostID.equals(postID)) {
                response.sendRedirect("user?command=ViewComment&pID=" + checkPostID);
            } else {
                if (clubID != null) {
                    int memberID = userDAO.getMemberID(user.getID(), clubID);
                    Post post = userDAO.getPost(clubID, postID);
...
//                request.setAttribute("myComment", myComment);
            ...
            if (!checkPostID.equals(postID)) {
                response.sendRedirect("user?command=SetToComment&pID=" + checkPostID);
            } else {
                if (clubID != null) {
                    Post post = userDAO.getPost(clubID, postID);
                    ArrayList<PostComment> postComment = userDAO.getPostComment(postID);
    ...
    private void LoadEditComment(HttpServletRequest request, HttpServletResponse response)
        ...
        String clubID = (String) session.getAttribute("clubID");
        String checkPostID = (String) session.getAttribute("postID");
        String checkPostCommentID = (String) session.getAttribute("postCommentID");

        UserDAO userDAO = new UserDAO();

        if (checkPostID == null || checkPostID.isBlank() || checkPostCommentID == null || checkPostCommentID.isBlank()) {
            if (clubID != null) {
                int memberID = userDAO.getMemberID(user.getID(), clubID);
                Post post = userDAO.getPost(clubID, postID);
                ArrayList<PostComment> postComment = userDAO.getPostComment(postID);
...
//                request.setAttribute("notMyComments", notMyComments);
                    request.setAttribute("commenterID", memberID);
                    session.setAttribute("postID", postID);
                    session.setAttribute("postCommentID", pcID);
                    request.getRequestDispatcher("user/ViewComment.jsp").forward(request, response);
                }
            } else {
                response.sendRedirect("user?command=Home");
            ...
            if (!checkPostID.equals(postID) || !checkPostCommentID.equals(pcID)) {
                response.sendRedirect("user?command=LoadEditComment&pID=" + checkPostID + "&pcID=" + checkPostCommentID);
            } else {
                if (clubID != null) {
                    int memberID = userDAO.getMemberID(user.getID(), clubID);
                    Post post = userDAO.getPost(clubID, postID);
...
//                int eventID = userDAO.checkEventID();
    ...
    private void EventManage(HttpServletRequest request, HttpServletResponse response)
        ...
        HttpSession session = request.getSession();
        session.removeAttribute("editEvent");
        String clubID = (String) session.getAttribute("clubID");
        String checkEventID = (String) session.getAttribute("eventID");
        ...
        if (checkEventID == null || checkEventID.isBlank()) {
            ...
            if (!checkEventID.equals(eventID)) {
                response.sendRedirect("user?command=EventManage&eID=" + checkEventID);
            } else {
                if (clubID != null) {
                    Event event = userDAO.getEvent(eventID);
                    if (event == null) {
        ...
        if (checkEventID == null || checkEventID.isBlank()) {
            if (clubID != null) {
                ...
                if (event == null) {
                    ...
                    request.setAttribute("eventID", eventID);
                    session.setAttribute("eventID", eventID);
                    session.setAttribute("editEvent", "editEvent");
                    request.getRequestDispatcher("user/EditEvent.jsp").forward(request, response);
                }
            } else {
                response.sendRedirect("user?command=Home");
            ...
            if (!checkEventID.equals(eventID)) {
                response.sendRedirect("user?command=LoadEditEvent&eID=" + checkEventID);
            } else {
                if (clubID != null) {
                    Event event = userDAO.getEvent(eventID);
                    if (event == null) {
    ...
    private void EventAttendeesList(HttpServletRequest request, HttpServletResponse response)
        ...
        if (checkEventID == null || checkEventID.isBlank()) {
            ...
            if (!checkEventID.equals(eventID)) {
                response.sendRedirect("user?command=EventAttendeesList&eID=" + checkEventID);
            } else {
                if (clubID != null) {
                    Event event = userDAO.getEvent(eventID);
                    ArrayList<User> eventAttendees = userDAO.getEventAttendeesList(eventID);
    ...
    private void LoadEditRating(HttpServletRequest request, HttpServletResponse response)
        ...
        User user = (User) session.getAttribute("account");
        String clubID = (String) session.getAttribute("clubID");
        String checkRatingID = (String) session.getAttribute("ratingID");

        UserDAO userDAO = new UserDAO();

        if (checkRatingID == null || checkRatingID.isBlank()) {
            if (clubID != null) {
                int memberID = userDAO.getMemberID(user.getID(), clubID);
                ArrayList<Rating> rating = userDAO.getRatingFromClubID(clubID);
                Rating myRating = userDAO.getMyRating(memberID);
            ...
            if (!checkRatingID.equals(ratingID)) {
                response.sendRedirect("user?command=LoadEditRating&rID=" + checkRatingID);
            } else {
                if (clubID != null) {
                    int memberID = userDAO.getMemberID(user.getID(), clubID);
                    ArrayList<Rating> rating = userDAO.getRatingFromClubID(clubID);
